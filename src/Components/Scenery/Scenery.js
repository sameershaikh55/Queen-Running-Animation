...
const Scenery = () => {
	...
	useLayoutEffect(() => {
		...
		var background1Movement = background1Animate.current.animate(
			sceneryFrames,
			sceneryBackgroundTiming
		);
		background1Movement.currentTime =
			background1Movement.effect.getTiming().duration / 2;

		var background2Movement = background2Animate.current.animate(
			sceneryFrames,
			sceneryBackgroundTiming
		);

		var foreground1Movement = foreground1Animate.current.animate(
			sceneryFrames,
			sceneryForegroundTiming
		);
		foreground1Movement.currentTime =
			foreground1Movement.effect.getTiming().duration / 2;

		var foreground2Movement = foreground2Animate.current.animate(
			sceneryFrames,
			sceneryForegroundTiming
		);
